    ...
    public function __construct(array $servicesConfig)
    {
        foreach ($servicesConfig as $serviceName => $serviceInfo) {
            if (!is_string($serviceName) || empty($serviceName)) {
                throw new \InvalidArgumentException('Name for a bitcoin blockchain explorer has to be specified.');
            }
            if (empty($serviceInfo['class'])) {
                throw new \InvalidArgumentException('Class for a bitcoin blockchain explorer has to be specified.');
            }
            if (empty($serviceInfo['code'])) {
                throw new \InvalidArgumentException('code for a bitcoin blockchain explorer has to be specified.');
            }
            if (empty($serviceInfo['label'])) {
                throw new \InvalidArgumentException('Label for a bitcoin blockchain explorer has to be specified.');
            }
        }
